...
public class BluetoothHandsfree {
	...
	private static final int PHONE_VT_RING_INFO = 13;				
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (intent.getAction().equals(Intent.ACTION_BATTERY_CHANGED)) {
                    ...
                    mHandler.sendMessage(msg);
                            ...
                            || intent.getAction().equals(BluetoothA2dp.ACTION_PLAYING_STATE_CHANGED)
                            ) {
                    ...
                    synchronized (BluetoothHandsfree.this) {
                        mA2dpState = state;
                        log("A2DP state is "+state+", old state="+oldState);
                        if (state == BluetoothProfile.STATE_DISCONNECTED) {
                            mA2dpDevice = null;
                        } else {
                            mA2dpDevice = device;
                        }
                        if (state != BluetoothA2dp.STATE_PLAYING/* oldState == BluetoothA2dp.STATE_PLAYING &&
        ...
        private synchronized void handlePreciseCallStateChange(boolean sendUpdate,
            ...
            if (mCallsetup != callsetup) {
                ...
                /* Modify for ALPS00303068: if sending RING without suspending A2DP, 
                   the ring tone in SE MW600 will be strange */
                if (mCall == 0) {
                    if ((mCallsetup == 1) && isA2dpMultiProfile() && 
                        ((mA2dpState == BluetoothA2dp.STATE_PLAYING) || 
                         (mA2dpState == BluetoothA2dp.STATE_NOT_PLAYING) || 
                         (mA2dpState == BluetoothA2dp.STATE_CONNECTED))) {
                        if (DBG) log("suspending A2DP stream for incoming call");
                        mA2dpSuspended = mA2dp.suspendSink(mA2dpDevice);                                    
                    } else if ((mCallsetup == 0) && mA2dpSuspended) {
                        if (isA2dpMultiProfile()) {
                            if (DBG) log("resuming A2DP stream after call setup failure");
        ...
        private void registerPhoneEvents(boolean register) {
            ...
            if(register) {
                ...
                if (FeatureOption.MTK_GEMINI_SUPPORT == true) {
                    /* +GEMINI SUPPORT */
                    /* Register for SIM1 */
                    mMCM.registerForServiceStateChangedGemini(mStateChangeHandler, SERVICE_STATE_CHANGED, null, Phone.GEMINI_SIM_1);                    
        	      ...
        	      // SH : Merge Yufeng's modification
                    ...
                    /* Register for SIM2 */
                    mMCM.registerForServiceStateChangedGemini(mStateChangeHandler, SERVICE_STATE_CHANGED2, null, Phone.GEMINI_SIM_2);                      
		      ...
		      if( FeatureOption.MTK_VT3G324M_SUPPORT == true )
		      {
                ...
                if (FeatureOption.MTK_GEMINI_SUPPORT == true) {
                    /* +GEMINI SUPPORT */
                    mMCM.unregisterForServiceStateChangedGemini(mStateChangeHandler, Phone.GEMINI_SIM_1);                    
                    ...
                    mMCM.unregisterForServiceStateChangedGemini(mStateChangeHandler, Phone.GEMINI_SIM_2);                     
    ...
    /* package */ synchronized boolean audioOn() {
        ...
        if (mA2dpSuspended == false) {
            ...
            else if (isA2dpMultiProfile() && (mA2dpState == BluetoothA2dp.STATE_NOT_PLAYING || mA2dpState == BluetoothA2dp.STATE_CONNECTED))
            {
                if (DBG) log("suspending A2DP stream for SCO, add by yi");
                mA2dpSuspended = mA2dp.suspendSink(mA2dpDevice);
            }
        }
        ...
        parser.register("+CNUM", new AtCommandHandler() {
            ...
            public AtCommandResult handleActionCommand() {
                ...
                if (FeatureOption.MTK_GEMINI_SUPPORT == true) {
                    ...
                    number = tm.getLine1NumberGemini(mPhonebook.getDefaultSIM());     
                } else {
