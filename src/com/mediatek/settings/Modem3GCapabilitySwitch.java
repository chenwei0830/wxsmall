...
public class Modem3GCapabilitySwitch extends PreferenceActivity implements Preference.OnPreferenceChangeListener{
    ...
    private static ProgressDialog pdSwitching = null;
    ...
    public Modem3GCapabilitySwitch(){
        instanceIndex = ++instanceFlag;
        Xlog.i(TAG, "Modem3GCapabilitySwitch(), instanceIndex="+instanceIndex);
    }
    ...
    private void update3GService(){
        ...
        if(CallSettings.isMultipleSim()){
            boolean isAirMode = Settings.System.getInt(getContentResolver(), Settings.System.AIRPLANE_MODE_ON, -1)==1;
    ...
    private void disSwitchProgressDialog() {
        ...
        if (pdSwitching != null && pdSwitching.isShowing()) {
            ...
            Xlog.d(TAG, "pdSwitching != null?"+(pdSwitching != null));
        }
    }
    ...
    private class MyHandler extends Handler {
        ...
        private void handleGetPreferredNetworkTypeResponse(Message msg) {
            ...
            Xlog.d(TAG, "-------------[arg1="+msg.arg1+"]--------------");
            Xlog.d(TAG, "-------------[arg2="+msg.arg2+"]--------------");
            Xlog.d(TAG, "-------------[MESSAGE_SET_PREFERRED_NETWORK_TYPE="+MESSAGE_SET_PREFERRED_NETWORK_TYPE+"]--------------");
            if (pd != null && pd.isShowing() && msg.arg2 == MESSAGE_SET_PREFERRED_NETWORK_TYPE) {
                try {
                    Xlog.d(TAG, "-------------[dismiss pd dialog]--------------");
                    pd.dismiss();
                } catch (Exception e) {
        ...
        private void handleSetPreferredNetworkTypeResponse(Message msg) {
            ...
            int slot = phoneMgr.get3GCapabilitySIM();
            if (CallSettings.isMultipleSim()) {
                mGeminiPhone.getPreferredNetworkTypeGemini(obtainMessage(MESSAGE_GET_PREFERRED_NETWORK_TYPE, 1001, MESSAGE_SET_PREFERRED_NETWORK_TYPE, null), slot);
            } else {
                mPhone.getPreferredNetworkType(obtainMessage(MESSAGE_GET_PREFERRED_NETWORK_TYPE, 1001, MESSAGE_SET_PREFERRED_NETWORK_TYPE, null));
            }
        }
    ...
    protected void onDestroy() {
        super.onDestroy();
        Xlog.d(TAG, "Instance["+instanceIndex+"]."+"onDestroy()");
    ...
    private void showInstanceIndex(String msg){
        if(DBG)Xlog.i(TAG, "Instance["+instanceIndex+"]: "+msg);
    }
}
