...
public class ListSimPreference extends ListPreference {
    ...
    public void onBindView(View contentView) {
        ...
        TelephonyManagerEx telephonyManagerEx = new TelephonyManagerEx(getContext());

        ImageView imageStatus = (ImageView)contentView.findViewById(R.id.simStatus);
        if (imageStatus != null) {
            int res = getSimStatusImge(telephonyManagerEx.getSimIndicatorStateGemini(mSimSlot));
            if(res == -1) {
        ...
        if (viewSim != null) {
            if((mSimColor >= 0) && (mSimColor <= 3)) {
                viewSim.setBackgroundResource(Telephony.SIMBackgroundRes[mSimColor]);
            } else {
