...
public class EmergencyCallHelper extends Handler {
    ...
    private void powerOnRadio() {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT
            ...
            log("Send broadcast to turn on radio with mode = " + mode);
    ...
    private void startRetryTimer() {
        ...
        if(FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            if (needSetDualSimMode(dualSimMode)) {
    ...
    private boolean needSetDualSimMode(int lastMode) {
        ...
        if (list == null || list.size() == 0) {
            if (FeatureOption.MTK_GEMINI_SUPPORT) {
                boolean isSimInserted = PhoneApp.getInstance().phoneMgr.isSimInsert(Phone.GEMINI_SIM_1);
                boolean isSim2Inserted = PhoneApp.getInstance().phoneMgr.isSimInsert(Phone.GEMINI_SIM_2);
                log("isSimInserted = " + isSimInserted + "  isSim2Inserted = " + isSim2Inserted);
                if (!isSimInserted && !isSim2Inserted) {
                    return true;
                }
            }
        }
