...
public class SipEditor extends PreferenceActivity
    ...
    enum PreferenceKey {
        ...
        PreferenceKey(int text, int initValue, int defaultSummary) {
            ...
            this.mDefaultSummary = defaultSummary;
        }
        ...
        void setValue(String value) {
            ...
            if (TextUtils.isEmpty(value)) {
                ...
                mPreference.setSummary(scramble(value));
    ...
    private boolean isEditTextEmpty(PreferenceKey key) {
        EditTextPreference pref = (EditTextPreference) key.mPreference;
        return TextUtils.isEmpty(pref.getText())
                || pref.getSummary().equals(getString(key.mDefaultSummary));
    }
    ...
    public boolean onPreferenceChange(Preference pref, Object newValue) {
        ...
        if (TextUtils.isEmpty(value)) {
            pref.setSummary(getPreferenceKey(pref).mDefaultSummary);
        } else if (pref == PreferenceKey.Password.mPreference) {
            pref.setSummary(scramble(value));
        } else {
            pref.setSummary(value);
        }

        if (pref == PreferenceKey.DisplayName.mPreference) {
    ...
    private PreferenceKey getPreferenceKey(Preference pref) {
        for (PreferenceKey key : PreferenceKey.values()) {
            if (key.mPreference == pref) return key;
        }
    ...
    private void loadPreferencesFromProfile(SipProfile p) {
        if (p != null) {
            ...
            try {
                ...
                for (PreferenceKey key : PreferenceKey.values()) {
                    Method meth = profileClass.getMethod(GET_METHOD_PREFIX
                            + getString(key.mText), (Class[])null);
            ...
            for (PreferenceKey key : PreferenceKey.values()) {
                key.mPreference.setOnPreferenceChangeListener(this);
                ...
                if (key.mInitValue != 0) {
                    key.setValue(getString(key.mInitValue));
                }
            }
    ...
    private void setupPreference(Preference pref) {
        ...
        for (PreferenceKey key : PreferenceKey.values()) {
            String name = getString(key.mText);
            if (name.equals(pref.getKey())) {
                key.mPreference = pref;
                return;
            }
        }
    }
    ...
    private void checkIfDisplayNameSet() {
        ...
        if (mDisplayNameSet) {
            PreferenceKey.DisplayName.mPreference.setSummary(displayName);
        } else {
