...
public class CallController extends Handler {
            ...
            case PHONE_STATE_CHANGED:
                ...
                if (state == Phone.State.IDLE) {
                    if (FeatureOption.MTK_GEMINI_SUPPORT) {
                        PhoneApp.getInstance().mCMGemini.unregisterForDisconnectGemini(this, Phone.GEMINI_SIM_1);
                        PhoneApp.getInstance().mCMGemini.unregisterForDisconnectGemini(this, Phone.GEMINI_SIM_2);
                    } else {
    ...
    public void placeCall(Intent intent) {
        ...
        boolean isEccCall = PhoneNumberUtils.isEmergencyNumber(number) 
            && !PhoneNumberUtils.isSpecialEmergencyNumber(number);
        
        Phone.State state = mCM.getState();
        if (isEccCall && state != Phone.State.IDLE) {
            ...
            if (fgCall != null && fgCall.getState().isAlive() 
                    && (!PhoneUtils.isEccCall(fgCall) 
                            || PhoneNumberUtils.isSpecialEmergencyNumber(activeCallAddress))) {
                if (FeatureOption.MTK_GEMINI_SUPPORT) {
                    PhoneApp.getInstance().mCMGemini.registerForDisconnectGemini(this, PHONE_STATE_CHANGED, 
                            null, Phone.GEMINI_SIM_1);
                    PhoneApp.getInstance().mCMGemini.registerForDisconnectGemini(this, PHONE_STATE_CHANGED, 
                            null, Phone.GEMINI_SIM_2);
                } else {
                    mCM.registerForDisconnect(this, PHONE_STATE_CHANGED, null);
                }
                mCallIntent = intent;
                try {
                    mCM.hangupAllEx();
    ...
    private CallStatusCode placeCallInternal(Intent intent) {
        ...
        if(PhoneApp.sGemini) {
            slot = intent.getIntExtra(Constants.EXTRA_SLOT_ID, Phone.GEMINI_SIM_1);
        }
        ...
        */
        ...
        if(FeatureOption.MTK_GEMINI_SUPPORT) {
            if(isEmergencyNumber)
                slot = pickBestSlotForEmergencyCall(number);
        }
        ...
        switch (callStatus) {
            case Constants.CALL_STATUS_DIALED:
                ...
                boolean voicemailUriSpecified = isVoicemailNumber(number, slot);
    ...
    int pickBestSlotForEmergencyCall(String number) {
        ...
        if(radioOn0 && radioOn1) {
            ...
            if (phone.getStateGemini(Phone.GEMINI_SIM_2) != Phone.State.IDLE) {
                ...
                if (state0 == ServiceState.STATE_OUT_OF_SERVICE && state1 == ServiceState.STATE_IN_SERVICE) {
                    slot = Phone.GEMINI_SIM_2;
                } else if (state1 == ServiceState.STATE_OUT_OF_SERVICE && state0 == ServiceState.STATE_IN_SERVICE) {
                    slot = Phone.GEMINI_SIM_1;
                } else {
                    if (isCDMASpecialEcc(number)) {
                        slot = Phone.GEMINI_SIM_2;
                    }
                }
            }
    ...
    private boolean isVoicemailNumber(String number, int slot) {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            voiceMailNumber = telephonyManager.getVoiceMailNumberGemini(Phone.GEMINI_SIM_1);
            voiceMailNumber2 = telephonyManager.getVoiceMailNumberGemini(Phone.GEMINI_SIM_2);
            if (slot == Phone.GEMINI_SIM_1) {
                ...
                if (voiceMailNumber2 != null
                        && PhoneNumberUtils.compare(voiceMailNumber2, number))
                    isVoicemail = true;
            }
        } else {
