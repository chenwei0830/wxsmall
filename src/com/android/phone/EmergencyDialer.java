 ...
 * You xx may obtain a copy of the License at
 *
...
public class EmergencyDialer extends Activity
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (Intent.ACTION_SCREEN_OFF.equals(intent.getAction())) {
                if(DBG) Log.d(LOG_TAG, "onReceive action screen off");
                finish();
            }
        }
    };
    ...
    public void afterTextChanged(Editable input) {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            String sim1State = SystemProperties.get(TelephonyProperties.PROPERTY_SIM_STATE);
            String sim2State = SystemProperties.get(TelephonyProperties.PROPERTY_SIM_STATE_2);
            if (DBG) log("afterTextChanged, sim1State: " + sim1State + ", sim2State: " + sim2State);
            if (sim1State.equals(SIM_STATE_PIN_REQUIRED) || sim1State.equals(SIM_STATE_PUK_REQUIRED)) {
                if (SpecialCharSequenceMgr.handleCharsForLockedDevice(this, input.toString(), this, Phone.GEMINI_SIM_1)) {
                    // A special sequence was entered, clear the digits
                    mDigits.getText().clear();
                }
            } else if (sim2State.equals(SIM_STATE_PIN_REQUIRED) || sim2State.equals(SIM_STATE_PUK_REQUIRED)) {
                if (SpecialCharSequenceMgr.handleCharsForLockedDevice(this, input.toString(), this, Phone.GEMINI_SIM_2)) {
                    // A special sequence was entered, clear the digits
                    mDigits.getText().clear();
                }
            }
            ...
            if (DBG) log("afterTextChanged, simState: " + simState);
            if (simState.equals(SIM_STATE_PIN_REQUIRED) || simState.equals(SIM_STATE_PUK_REQUIRED)) {
                if (SpecialCharSequenceMgr.handleCharsForLockedDevice(this, input.toString(), this, Phone.GEMINI_SIM_1)) {
            // A special sequence was entered, clear the digits
            mDigits.getText().clear();
        }
    ...
    protected void onCreate(Bundle icicle) {
        ...
        if (DBG) Log.d(LOG_TAG, "add show when locked flag");
