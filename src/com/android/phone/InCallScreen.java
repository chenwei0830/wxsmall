   ...
   /**
    ...
    protected void onResume() {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            int simid = -1;
            int messageid = -1;
            if (!((GeminiPhone)mPhone).getPendingMmiCodesGemini(Phone.GEMINI_SIM_1).isEmpty()) {
                simid = Phone.GEMINI_SIM_1;
                messageid = PhoneApp.MMI_CANCEL;
            } else if (!((GeminiPhone)mPhone).getPendingMmiCodesGemini(Phone.GEMINI_SIM_2).isEmpty()) {
                simid = Phone.GEMINI_SIM_2;
                messageid = PhoneApp.MMI_CANCEL2;
            }
            if (-1 != simid) {
    ...
    protected void onPause() {
        ...
        if (isNeedToUpdateInCallNotification()) {
            if (DBG) log("Screen is on && not the top activity!");
            mApp.notificationMgr.updateInCallNotification();
        } else {
            if (DBG) log("don't update the incallscreen.");
        }
    ...
    /* package */ void updateKeyguardPolicy(boolean dismissKeyguard) {
        if (DBG) log("incallscreen: updateKeyguardPolicy() ,dismissKeyguard: "+dismissKeyguard);
        if (dismissKeyguard) {
            if (DBG) log("updateKeyguardPolicy: set dismiss keyguard window flag ");
            getWindow().addFlags(WindowManager.LayoutParams.FLAG_DISMISS_KEYGUARD);
        } else {
            if (DBG) log("updateKeyguardPolicy: clear dismiss keyguard window flag ");
        ...
        if (action.equals(intent.ACTION_MAIN)) {
            ...
            if (FeatureOption.MTK_VT3G324M_SUPPORT == true) {
                ...
                if (mCM.getState() == Phone.State.RINGING) {
                    ...
                    if (mCM.getState() != Phone.State.IDLE && VTCallUtils.isVideoCall(mCM.getActiveFgCall())) {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if (VTCallUtils.VTScreenMode.VT_SCREEN_CLOSE == mVTInCallScreen.getVTScreenMode()) {
                ...
                if (FeatureOption.MTK_GEMINI_SUPPORT) {
                    ...
                    if( null != PhoneApp.getInstance().phoneMgr){
                        if (!PhoneApp.getInstance().phoneMgr.isSimInsert(Phone.GEMINI_SIM_1) &&
                            ...
                            ///M: #Only emergency# text replace in case IMEI locked @{
                            showCanDismissDialog(mServiceStateExt.isImeiLocked() ?
...
//MTK add below one line:
     ...
     */
    static public boolean getmLocaleChangedOnlyForTest()
    {
     ...
     */
    /* package */ PhoneApp getmAppOnlyForTest() {
        return mApp;
    }
...
/* need to review if should check phone 2 or not */
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        if (mVTInCallScreen.onPrepareOptionsMenu(menu)) {
            return true;
        }
        if (ViewConfiguration.get(this).hasPermanentMenuKey()) {
    ...
    /*void handlePendingStatus() {
        ...
        if (msg != null) {
            // !!!! Need to check again, google delete onMMIInitiate()
            //onMMIInitiate((AsyncResult)msg.obj, msg.arg1);
        }
    }*/
    ...
    void internalAnswerCallForDualTalk() {
        ...
        try {
            if (callCount > 2) {
                ...
                if(FeatureOption.MTK_VT3G324M_SUPPORT == true) {
                    if(VTCallUtils.isVideoCall(ringing) && ringing.isRinging()) {
                        mVTInCallScreen.internalAnswerVTCallPre();
                    }
                }
                PhoneUtils.answerCall(ringing);
